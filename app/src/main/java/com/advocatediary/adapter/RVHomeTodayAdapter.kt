...
import com.advocatediary.model.home.HomeTodayCasesArr
...
class RVHomeTodayAdapter(
    val context: Context,
    val listToday: List<HomeTodayCasesArr>?,
    var purposeList: List<CasePurposesDatum>,
    var clickListener: OnclickListener
) :
    RecyclerView.Adapter<RVHomeTodayAdapter.MyViewHolder>() {

    private  var lastAnimatedPosition: Int?=0

    override fun onBindViewHolder(holder: MyViewHolder, p1: Int) {

        holder.iv_view_line.setBackgroundColor(context.getColor(R.color.first_tab_color_home))
        holder.swipe_layout.setLockDrag(false)
        holder.tv_today_name.text = listToday!!.get(p1).getClient()!!.getFullName()

        var value: String = listToday!!.get(p1).getClient()!!.getFullName()!!
        ...
        holder.tv_case_type.text = listToday!!.get(p1).getCase()!!.getCaseType()!!.getCaseType()

        holder.tv_case_address.text = listToday!!.get(p1).getCase()!!.getCourt()!!.getCourtTitle()

        holder.tv_place_data.text =
            listToday!!.get(p1).getCase()!!.getDistrict()!!.getTitle() + ", " + listToday!!.get(p1).getCase()!!.getState()!!.getTitle()
        ...
        holder.linear_home_todayd.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View?) {
                ...
                intent.putExtra("case_id", listToday!!.get(p1).getCaseId())
        ...
        holder.tv_purpose_evidance.text = listToday!!.get(p1).getPurposeHearing()!!.getName()

        holder.tv_purpose_evidance.setTextColor(context.getColor(R.color.first_tab_color_home))
        holder.update_layout.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View?) {
                ...
                tv_next_date_update.setOnClickListener(object : View.OnClickListener {
                    override fun onClick(v: View?) {
                        showDatePickar(tv_next_date_update, listToday.get(p1).getCaseNextDate()!!)
                    }
                ...
                bt_update_case.setOnClickListener(object : View.OnClickListener {
                    override fun onClick(v: View?) {
                        ...
                        jsonObject.put("case_next_date", listToday!!.get(p1).getCaseNextDate());
                        jsonObject.put("case_id", listToday!!.get(p1).getCaseId());

                        clickListener.onClcikListener(jsonObject.toString(), p1)
    ...
    class MyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var update_layout = itemView.findViewById<FrameLayout>(R.id.delete_layout) as FrameLayout
